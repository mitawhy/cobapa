...
# Halaman Prediksi
    ...
    # Dropdown untuk kolom "Age"
    Education_Level = st.selectbox('Input Education Level', [i for i in sorted(df['Education Level'].unique())])

    # Dropdown untuk kolom "Institution Type"
    Institution_Type = st.selectbox('Input Institution Type', [i for i in sorted(df['Institution Type'].unique())])

    # Dropdown untuk kolom "IT Student"
    IT_Student = st.selectbox('Input IT Student', [i for i in sorted(df['IT Student'].unique())])
    ...
    Financial_Condition = st.selectbox('Input Financial Condition', [i for i in sorted(df['Financial Condition'].unique())])

    # Dropdown untuk kolom "Internet Type"
    Internet_Type = st.selectbox('Input Internet Type', [i for i in sorted(df['Internet Type'].unique())])

    # Dropdown untuk kolom "Network Type"
    Network_Type = st.selectbox('Input Network Type', [i for i in sorted(df['Network Type'].unique())])

   # Dropdown untuk kolom "Class Duration"
    Class_Duration = st.selectbox('Input Class Duration', [i for i in sorted(df['Class Duration'].unique())])

    # Dropdown untuk kolom "Self Lms"
    Self_Lms = st.selectbox('Input Self Lms', [i for i in sorted(df['Self Lms'].unique())])
